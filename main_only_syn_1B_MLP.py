        ...
        #for e in range():
            ...
            for x_true1, x_true2 in self.dataloader:
                ...
                if self.args.sample == "sample":
                    ...
                    x_true2 = x_true2.view([self.batch_size, 4096])
                ...
                if self.args.policy == "greedy":
                    best_ai = greedy_policy_Smax_discount(self.z_dim, mu_prime, logvar_prime,alpha=self.omega)
                    #print("step {}, latents {}".format(step, best_ai))
                    c.update(best_ai)

                if self.args.policy == "e-greedy":
                    best_ai = e_greedy_policy_Smax_discount(self.z_dim, mu_prime, logvar_prime, alpha=self.omega, epsilon=self.epsilon)
                    #print("step {}, latents {}".format(step, best_ai))
                    c.update(best_ai)

                # Step 2: compute the Imax
                mu_syn = mu_prime[:, best_ai]
                logvar_syn = logvar_prime[:, best_ai]
                ...
                print("encoder.10.weight first 10 samples {}".format(
                    self.optim_VAE.param_groups[0]['params'][4][:10, :10]))
                print()
